...
** Last update Sun Jan 10 01:30:57 2016 Antoine Baché
*/
...
/*
** Pas la peine de verif les infos du header, le chksum le fait
*/
...
int		unarchive(char *av)
{
  ...
  if (check_rights_unarchive(av) == 1)
    return (1);
  if ((fd = open(av, O_RDONLY)) < 0 || read_header(fd, &head) == 1)
    {
...
{
  if (*env == NULL || ac != 2 || unarchive(av[1]) == 1)
    {
